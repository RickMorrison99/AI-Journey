...
Cite by name with their relevant concept: Kent Beck (3X, TDD, XP), Michael Feathers (seam-based legacy modernisation, characterisation tests), Nicole Forsgren (DORA, SPACE, Accelerate), Addy Osmani (spec-driven AI development, 30-40% spec rule), Simon Wardley (evolution mapping, doctrine), Dave Farley (continuous delivery, deployment pipeline), Charity Majors (observability, production excellence), Martin Fowler (refactoring, DRY, patterns), Laura Nolan (SRE, blast radius), Gregor Hohpe (enterprise integration, architecture), Gene Kim (DevOps, The Phoenix Project), Jez Humble (continuous delivery), Sam Newman (microservices, strangler fig), Mary Poppendieck (lean software), David J. Anderson (Kanban Method, WIP limits, classes of service), Don Reinertsen (cost of delay, batch size, product development flow), Daniel Vacanti (flow metrics, Monte Carlo forecasting, cycle time scatter plots), Henrik Kniberg (Spotify model, done-done, visualising flow), Allen Holub (no estimates, flow-first agile, throughput over velocity), Tom & Mary Poppendieck (lean software development, muda, decide as late as possible), Mike Cohn (INVEST criteria, user stories, story mapping), Ron Jeffries (3 Cs: Card/Conversation/Confirmation, YAGNI, Dark Scrum critique), Marty Cagan (empowered product teams, continuous discovery, feature factory anti-pattern), Teresa Torres (opportunity solution trees, continuous discovery habits), Michael Nygard (circuit breakers, timeouts, bulkheads, stability patterns), Len Bass / Paul Clements / Rick Kazman (quality attribute workshops, architecture trade-off analysis, NFRs as architectural drivers), and the TM Forum Open Digital Architecture team.
